...
const stkPushCallback = async(req, res) => {
    try{
       ...
       // save mpesa transaction in db
       const mpesaTransaction = await mpesaService.createMpesaTransaction({
        Order_ID, MerchantRequestID, CheckoutRequestID, ResultCode, 
        ResultDesc, PhoneNumber, Amount, MpesaReceiptNumber, TransactionDate
       })

       res.status(201).json({
        status: "mpesa transaction created successfully",
        mpesaTransaction
       })

       console.log("mpesa transaction", mpesaTransaction)
